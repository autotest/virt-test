...
class VersionableClass(object):
    ...
    def __new__(cls, *args, **kargs):
        """
        If this method is invoked it means that something goes wrong because
        this class should be replaced by :class:`Manager` factory.
        """
        raise Exception("Class %s is not prepared for using. It has to be used"
                        " versionable_class.factory(cls) for class"
                        " preparation." % (cls))
