...
class Serial(base.TypedDeviceBase):
    ...
    # Helper for removing any/all <source mode=*> elements
    def nuke_source_bind_connect(self):
        # Don't assume there's only one
        del_list = []
        for source in self.xmltreefile.getiterator('source'):
    ...
    # Override source_path setter to first remove all previous source elements
    def set_source_path(self, value):
        self.nuke_source_bind_connect()
        self.nuke_source_paths()
    ...
    def get_source_connect(self):
        try:
            ...
            source_dict = dict(source_connect.items())
            # Remove mode since it's implied
            del source_dict['mode']
            return source_dict
