...
// Globals
char shell_port[6] = "10022";
char file_transfer_port[6] = "10023";
...
SOCKET PrepareListenSocket(char *port)
{
	int RetVal;
	int ipv6only, reuseaddr;
	...
	setsockopt(ServSock, SOL_SOCKET, SO_LINGER, (char *)&l, sizeof(l));
	setsockopt(ServSock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&ipv6only, sizeof(ipv6only));
	setsockopt(ServSock, SOL_SOCKET, SO_REUSEADDR, (char *)&reuseaddr, sizeof(reuseaddr));

	if (bind(ServSock, AddrInfo->ai_addr, AddrInfo->ai_addrlen) == SOCKET_ERROR)
	{
		ExitOnError("bind error", TRUE);
	}
	if (listen(ServSock, 5) == SOCKET_ERROR)
	{
		ExitOnError("Listen socket error", TRUE);
	}
	...
	while (1)
	{
		...
		{
			ExitOnError("select() failed with error");
		}
	}
...
client_info* Accept(SOCKET ServSock)
{
    ...
    ci->socket = ConnSock;
    sprintf(ci->addr_str, "%s", Hostname);
...
BOOL SendFileChunks(client_info *ci, const char *filename)
{
	...
	if (!fp)
	{
		AppendMessage("Unable to read file %s, error code: %d", filename, fp);
		return FALSE;
	}
...
BOOL ReceiveFileChunks(client_info *ci, const char *filename)
{
	...
	if (!fp)
	{
		AppendMessage("can not create file %s, error code %d", filename, fp);
		return FALSE;
	}
                   ...
                   LPSTR lpCmdLine, int nShowCmd)
{
    ...
    sprintf(title, "Remote Shell Server (listening on ports %s, %s)",
