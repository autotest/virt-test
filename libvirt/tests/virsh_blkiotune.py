...
def check_blkiotune(params):
    ...
    cmd_output = result.stdout.strip().split("\n")
    for l in cmd_output:
        k, v = l.split(':')
    ...
    # To change a running guest with 'config' option, which will affect next boot,
    # if don't shutdown the guest, we need to run virsh dumpxml with 'inactive'
    # option to get guest XML changes.
    ...
    if cgconfig == "on" and vm and vm.is_alive():
        blkio_params_from_cgroup = get_blkio_params_from_cgroup(vm_name)
...
def get_blkio_params_from_cgroup(vm_name, qemu_path="/libvirt/qemu/", ):
    """
...
def run_virsh_blkiotune(test, params, env):
    ...
    1) Positive testing
       1.1) get the current blkio parameters for a running/shutoff guest
           1.1.1) with valid option
       1.2) set the current blkio parameters for a running/shutoff guest
           1.2.1) set valid 'weight' parameters and option
           1.2.2) set valid 'device-weights' parameters and option
    2) Negative testing
       ...
       2.2) set blkio parameters running/shutoff guest
           2.2.1) invalid 'weight' parameters or/and invalid option
           2.2.2) invalid 'device-weights' parameters or/and invalid option
           2.2.3) stop cgroup service
    """
    ...
    if status_error == "no":
        if change_parameters == "no":
            get_blkio_parameter(_params)
        else:
            set_blkio_parameter(_params)
