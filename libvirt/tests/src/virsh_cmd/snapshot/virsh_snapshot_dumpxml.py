...
def get_snap_createtime(vm_name, snap_name):
    ...
    pat = re.compile('%s *\d{4}-\d+-\d+\s\d{2}:\d{2}:\d{2}' % snap_name)
    data = re.search(pat, result.stdout).group()
...
def snapshot_dumpxml_check(output, opt_dict):
    ...
    if opt_dict.has_key('disk_opt'):
        dstate_snip = "<state>disk-snapshot</state>"
    else:
        dstate_snip = "<state>%s</state>" % \
                      "".join(opt_dict['dom_state'].split())
...
def run_virsh_snapshot_dumpxml(test, params, env):
    ...
    try:
        ...
        vmxml_backup.undefine("--snapshots-metadata")
        vmxml_backup.define()
        logging.debug("xml is %s", vmxml_backup.dict_get('xml'))

        # Delete useless disk snapshot file
        dom_xml = vmxml_backup.dict_get('xml')
        disk_path = dom_xml.find('devices/disk/source').get('file')
        snap_disk_path = disk_path.split(".")[0] + "." + snap_name
