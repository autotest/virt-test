...
def run_virsh_domfstrim(test, params, env):
    ...
    try:
        ...
        ori_diskmap = utils.run(map_cmd).stdout.strip('\n\x00')
        def get_diskmap_size(diskmap):
            """
            Collect size from disk map
            :param diskmap: map of scsi disk
            :return: disk size
            """
            sum = 0
            ...
            return sum
        ori_size = get_diskmap_size(ori_diskmap)
        logging.debug("origin disk map is %s", ori_diskmap)
        ...
        def _full_mapped():
            ...
            """
            global full_size
            full_diskmap = utils.run(map_cmd).stdout.strip('\n\x00')
            full_size = get_diskmap_size(full_diskmap)
            logging.debug("Disk map after dd command is %s, %s", full_diskmap)
        ...
        def _trim_completed():
            ...
            empty_diskmap = utils.run(map_cmd).stdout.strip('\n\x00')
            empty_size = get_diskmap_size(empty_diskmap)
            logging.debug("Disk map after remove is %s.", empty_diskmap)
