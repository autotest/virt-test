...
class RebaseTest(qemu_disk_img.QemuImgTest):
      ...
      def rebase(self, t_params={}):
          """
          rebase snapshot, AKA changes backingfile to new image;
          """
      ...
      def check_backingfile(self):
          ...
          if backingfile:
              if not (self.base_image_filename in backingfile.group(0)):
                  msg = "Expect backingfile: %s" % self.base_image_filename
                  msg += "Actuary backingfile: %s" % backingfile
                  raise error.TestFail(msg)
          else:
               msg = "image '%s' not found backingfile" % self.image_filename
               raise error.TestFail(msg)
...
def run_qemu_disk_img_rebase(test, params, env):
    ...
    params.update(
        {"image_name_%s" % base_image: params.get("image_name"),
         "image_format_%s" % base_image: params.get("image_format")})
    ...
    for idx, tag in enumerate(image_chain):
        ...
        t_file = params.get("guest_file_name_%s" % tag)
