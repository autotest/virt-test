...
def run_sr_iov_hotplug_negative(test, params, env):
    ...
    def make_pci_add_cmd(pa_pci_id, pci_model, pci_addr="auto"):
        pci_add_cmd = ("pci_add pci_addr=%s host host=%s,if=%s" %
                       (pci_addr, pa_pci_id, pci_model))
        if params.get("hotplug_params"):
            pci_add_cmd += add_params()
        return pci_add_cmd


    def make_device_add_cmd(pa_pci_id, pci_model, pci_addr=None):
    ...
    if modprobe_cmd:
        ...
        utils.system_output(modprobe_cmd)
    ...
    for j in range(rp_times):
        ...
        try:
            ...
            error.context(msg, logging.info)
            case_fail = False
            vm.monitor.send_args_cmd(pci_add_cmd, convert=False)
            case_fail = True
        except Exception, e:
            if neg_msg:
                ...
                if neg_msg not in str(e):
                    msg = "Could not find '%s' in msg '%s'" % (neg_msg, e)
                    raise error.TestFail(msg)
            logging.debug("Could not hotplug device to guest, %s" % e)
        if case_fail:
