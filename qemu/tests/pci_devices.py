...
def process_qtree(qtree):
    ...
    for node in qtree.get_nodes():
        if node.parent and node.parent.qtree.get('type') in ('PCI', 'PCIE'):
            ...
            class_name = node.qtree.get('class_name')
            class_name = {'VGA controller': 'VGA compatible controller',
                          'SCSI controller': 'SCSI storage controller'
                          }.get(class_name, class_name)
            qtree_pciinfo.append({'class_name': class_name,
