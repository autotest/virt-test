...
from virttest import utils_misc, utils_test, aexpect, storage, data_dir
...
def run_pci_hotplug(test, params, env):
    ...
    def device_add_block(pci_num):
        ...
        image_format = params["image_format_%s" % img_list[pci_num+1]]
        if not image_format:
    ...
    qemu_binary = utils_misc.get_path(test.bindir, "qemu/%s" % qemu_binary)
    ...
    pci_num_range = int(params["pci_num"])
    rp_times = int(params["repeat_times"])
    img_list = params["images"].split()
    for j in range(rp_times):
        ...
        for pci_num in xrange(pci_num_range):
            sub_type = params["sub_type_before_plug"]
            if sub_type:
                error.context("Run sub test '%s' before hotplug" % sub_type,
                              logging.info)
            ...
            sub_type = params["sub_type_after_plug"]
            if sub_type:
                error.context("Run sub test '%s' after hotplug" % sub_type,
                              logging.info)
                utils_test.run_virt_sub_test(test, params, env, sub_type)
        for pci_num in xrange(pci_num_range):
            sub_type = params["sub_type_before_unplug"]
            if sub_type:
                error.context("Run sub test '%s' before hotunplug" % sub_type,
                              logging.info)
                utils_test.run_virt_sub_test(test, params, env, sub_type)
            ...
            sub_type = params["sub_type_after_unplug"]
            if sub_type:
                error.context("Run sub test '%s' after hotunplug" % sub_type,
                              logging.info)
                utils_test.run_virt_sub_test(test, params, env, sub_type)
