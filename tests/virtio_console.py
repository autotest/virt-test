...
def run_virtio_console(test, params, env):
    ...
    def test_interrupted_transfer():
        ...
        if interruption == 'stop':
            ...
            ret = session.cmd_status(params.get("s3_support_chk_cmd"))
            if ret:
                raise error.TestNAError("Suspend to mem (S3) not supported.")
            set_s3_cmd = params.get('s3_start_cmd')
        elif interruption == 's4':
            interruption = _s4
            session = vm.wait_for_login()
            ret = session.cmd_status(params.get("s4_support_chk_cmd"))
            if ret:
                raise error.TestNAError("Suspend to disk (S4) not supported.")
            set_s4_cmd = params.get('s4_start_cmd')
    ...
    def test_perf():
        ...
        for param in test_params.split(';'):
            ...
            try:
                ...
                thread = kvm_virtio_port.ThRecv(port.sock, exit_event,
                                                 buf_len)
                thread.start()
                loads.start()
