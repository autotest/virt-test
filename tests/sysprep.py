...
def run_sysprep(test, params, env):
    ...
    unattended_file_link = os.path.join(test.virtdir, "unattended",
                                        unattended_file)
    ...
    logging.info("Output from check System ID command: %s" % output)
    try:
        sid = re.findall(re_sid, output)[0]
    except IndexError:
        raise error.TestFail("Fail to get guest's System ID")
    ...
    logging.info("Run sysprep command in guest. %s" % sysprep_cmd)
    session.sendline(sysprep_cmd)
    logging.info("Waiting guest power down.....")
    s = utils_misc.wait_for(vm.is_dead, timeout * 3, 3)
    if not s:
    ...
    for vm_i in vms:
        ...
        try:
            ...
            raise error.TestFail("Fail to get System ID of %s" % vm_i.name)
        logging.info("VM:%s System ID is: %s" % (vm_i.name, sid))
        if sid in sids.keys():
            logging.error("VM: %s have duplicate System ID: %s" % (vm_i.name,
                                                                   sid))
            sid_same.append(sid)
