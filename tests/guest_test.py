...
def run_guest_test(test, params, env):
    ...
    try:
        ...
        dst_rsc_path = params.get("dst_rsc_path", "C:\script.au3")
        ...
        elif params.get("remote_copy") == "yes":
            # copy file from host to guest
            rsc_path = os.path.join(test.bindir, params.get("rsc_path"))
            if not os.path.exists(rsc_path):
                raise error.TestError("Resource %s does not exists."% rsc_path)

            dst_rsc_dir = params.get("dst_rsc_dir", "~")

            if os.path.isdir(rsc_path):
                ...
                dst_rsc_path = os.path.join(dst_rsc_dir,
                                            os.path.basename(rsc_path),
                                            rsc_entry)

            elif os.path.isfile(rsc_path):
                ...
                raise error.TestError("Illegal resource path.")
        else:
            session.cmd_output("del %s" % dst_rsc_path, internal_timeout=0)
            script_path = virt_utils.get_path(test.virtdir, script)
...
def run_guest_test_background(test, params, env, test_name=""):
    """
