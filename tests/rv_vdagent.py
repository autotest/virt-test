...
def run_rv_vdagent(test, params, env):
    ...
    if vdagent_test == "start":
        ...
        if "running" in status:
            ...
            raise error.TestFail("Vdagent status is not running after a start attempt.")
    #stop test
        ...
        if "stopped" in status:
            ...
            print "Status: " + status
            raise error.TestFail("Vdagent status is not stopped after a stop attempt.")
    #restart test when vdagent service is running
        ...
        if "running" in status:
            ...
            raise error.TestFail("Vdagent status is not started after a restart attempt.")
