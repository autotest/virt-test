...
def run_trans_hugepage_relocated(test, params, env):
    ...
    def nr_hugepage_check(sleep_time, wait_time):
        time_last = 0
        while True:
            value = int(utils_misc.get_mem_status("AnonHugePages"))
    ...
    free_memory = utils_misc.get_mem_status("MemFree")
    total_memory = utils_misc.get_mem_status("MemTotal")
    swap = utils_misc.get_mem_status("SwapTotal")
    hugepage_size = utils_misc.get_mem_status("Hugepagesize")
    mem = params.get("mem")
    vmsm =  int(mem) + 128
