...
from autotest.client.tests.kvm.tests import qemu_guest_agent


class QemuGuestAgentSnapshotTest(qemu_guest_agent.QemuGuestAgentBasicCheck):
    @error.context_aware
    ...
    def _action_after_fsthaw(self, *args):
        error.context("Check if the created exists in the guest.")
        session = self._get_session(self.params, None)
